...
export class AssetStore {
  ...
  async add(blob, name = "图片", size) {
    ...
    if (!this.assets.has(id)) this.assets.set(id, { id, blob, url: URL.createObjectURL(blob), name, ...dimensions });
    return this.get(id);
  }
  async encode(ids) {
  ...
  collect(conversations) {
    ...
    for (const c of conversations) {
      [c.mainId, ...c.pinnedIds, ...c.attachments].filter(Boolean).forEach(id => used.add(id));
