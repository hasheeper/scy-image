# Scylla Image

NAI 绘图与对话绘图工作区。纯静态、运行时零依赖、零构建，可直接部署到 GitHub Pages，也可用本地 Node 服务运行。

## 对话绘图

- 从顶栏「对话绘图」打开 `chat.html`。与 NAI 工作区互相在新标签页打开，保留各自的内存会话。
- 支持目录中的 GPT Image 2、2.5 Sunburst / Flare 与 Nano Banana 系列，按模型显示质量、画幅、分辨率和格式；Lite 仅开放 1K。
- 图片可上传、粘贴或拖入；上一张结果自动成为下一轮底图，你上传的原图会继续留在上下文里，形成「原图＋结果」两张图一个会话，不会被结果顶掉。不需要时可随时移除，移除后不会在下一轮被重新带回。
- 发送即消耗输入栏图片：图片随该轮请求进入历史记录的引用行，失败或取消也不放回输入栏。底图与参考图在上下文面板中管理，结果图不会出现在输入栏。
- 上下文面板分区显示：容量计（`已带/上限`）、本轮编辑的底图、附带的参考图、实际带入的文字。每张图标注是「你上传的」还是「模型结果」，可上移下移调整顺序、设为底图、固定或移除。带入轮数设为 0 表示当前分支全部文字。首版使用历史文字＋底图＋参考图直接绘图，不额外调用聊天模型，不冒充服务端原生会话。
- 带图数量按模型能力分级：GPT Image 2 系列 15 张、Nano Banana 14 张、2.5 Flash 3 张、Lite 仅 1 张（上限取模型规则与网关 `max_context_images` 的较小值）。切到更窄的模型时立即提示会丢几张，超出上限的图仍在面板中以虚线灰显标注「不带入」，不静默丢弃也不在发送时才报错。
- 从任意旧图「继续编辑」形成分支；重新生成保留同一轮的多个版本。失败或取消不会成为有效上下文。
- 单张上传不超过 8MB，本轮总图片不超过 32MB；数量按模型与网关限制取小。上传图片和会话只在内存，刷新前提示，刷新后清空。
- 顶栏额度计按供应商命名与标注：OpenAI 用其官方结绳标记，Gemini／NovelAI 用四角星；付费时切换为跨供应商的「积分」。余量≤25%／≤10% 时图标、数字与细条一起转为警告与危险色，额度未知则整体转灰。
- 页面复用 NAI 的在线字体、主题、SVG 图标、暗色画框、顶部细进度条和单道掠光。手机工具收进侧栏，输入栏适配可见视口与安全区。
- NAI 绘图、API Key、删除对话统一为顶栏图标；手机端从顶栏菜单展开，图标位于会话列表上方，不再占据侧栏底部。
- 输入栏采用发送／停止 SVG 图标；附件仅展示圆角缩略图与右上角关闭，底图和固定操作收进上下文面板。上下文以图标＋数量显示，超出模型上限时附 `−N` 提示，保留悬停说明与无障碍名称。
- 消息区不显示轮次关系、重复身份或引导提示；图片仅保留模型、尺寸、必要费用与操作，多版本时才显示切换控件。

## 新版媒体接口与费用

- 生成与报价分别使用 `/v1/studio/image/generate`、`/v1/studio/image/estimate`；改图使用 `/v1/image/tools/run`、`/v1/image/tools/estimate`。
- 模型使用目录返回的真实 ID（含 `@local`），旧 NAI 偏好按唯一匹配迁移。
- `/v1/media/quota` 按模型 `quota_group` 读取周图片额度，媒体积分独立显示。临时 Key 的日请求额度仍是另一项限制，不与媒体额度混算。
- 顶栏跟随选中模型显示免费图片余量；当前报价需要积分时显示媒体积分。前端不再用 `rpd_units / rpd` 估价或自行扣点，完成后重读服务器额度。
- 每次执行重新报价，并携带 `max_cost`。付费需明确确认；免费转付费或报价上涨会停止本轮/剩余队列，不自动重试生成请求。取消不保证上游已经停止或退费。
- 两个工作区共用同源 Web Lock，本地代理另加进程锁；不支持安全锁的浏览器拒绝提交。不能约束其他设备或其他站点使用同一 Key 的请求。
- 不承诺新路径完全不消耗临时请求额度；最终计费以服务器账本为准。原生 Responses/Gemini 会话、渐进预览、远程结果 URL 和异步任务轮询尚未启用。

## 两种运行方式

**GitHub Pages（浏览器直连）**

Settings → Pages → Source 选 **Deploy from a branch**，Branch 选 `main` + `/docs`。
（Pages 只支持仓库根目录或 `/docs`，这是站点放在 `docs/` 的原因。）

上游已开放 CORS，浏览器直接请求，无需任何中转服务。Key 保存在你自己的浏览器里。
所有资源引用均为相对路径，因此在 `user.github.io/repo/` 这类子路径下同样可用。

**本地（后端持有 Key）**
```bash
# 在 config/api-key.txt 写入 token，然后
npm start   # → http://127.0.0.1:3215
```
此模式下 Key 只由本机后端读取，不进入浏览器。前端会自动探测并切换。
需要 Node 18+，无需 `npm install`。

## Key 的存放与加密

首次打开会要求填写 Key，两种保存方式：

- **口令加密保存** — PBKDF2-SHA256（21 万次迭代）派生密钥，AES-GCM 加密后存入
  localStorage。之后每次打开输入口令解锁；解密后的 Key 只存在内存变量中。
- **仅本标签页** — 明文存 sessionStorage，关闭标签即消失，不落盘、无需口令。

关于加密边界，说清楚比说好听重要：

| 能防 | 不能防 |
| --- | --- |
| 他人翻看本机 localStorage | 页面遭 XSS 或恶意扩展在解锁状态下读取内存明文 |
| 浏览器配置同步外泄 | 解锁后有人 hook `fetch` |
| Key 被误提交进 git | — |

静态站点最终必须以明文把 Key 发给上游，这是原理限制，客户端加密无法消除。模型和
生成接口使用请求头；Scylla 的全局额度接口目前只接受查询参数。如果你不信任部署环境，
请使用本地后端模式，让浏览器完全不接触 Key。

顶栏「锁定」按钮的行为取决于保存方式：

- 口令加密模式 — 丢弃内存中的 Key，下次打开需重新输入口令。
- 仅本标签页模式 — 没有口令可验证，因此会直接清除 Key，需重新填写（会先确认）。
- 本地后端模式 — 浏览器从不持有 Key，按钮隐藏。

## 默认行为

- **描述可留空** — 不填写 Prompt 也可直接生成；填写时仍按原样发送，不做改写。
- **不缓存到服务端**（`cache: false`）— 每次都真实生成。
- **历史仅存于本次会话** — 默认保留最近 25 个结果，可自定义数量（0 为无限）；关闭
  「加入历史」时仅保留最新 1 张。可点击缩略图回看、
  一键「复用参数」、单张删除或清空。**刷新后清空**（图片只在内存，不落盘）。
  清空和刷新前都会弹出确认，避免误触丢图。
- **不压缩**（`optimize: false`）— 返回全质量 PNG。开启后上游会转为 JPEG（有损）。
- **不改写 Prompt**（`transform_prompt: false`）— 不经过任何 LLM 处理。

## 从图片导入参数

- **拖到页面任意位置**即可读取：外部 PNG / JPEG，或直接拖动右侧历史缩略图。
  顶栏的导入按钮也可打开文件选择。
- 识别两种写法：NovelAI 的 PNG `Comment` 块（含 `zTXt` / `iTXt` 压缩形式与
  JPEG EXIF），以及 WebUI / A1111 的 `parameters` 块。
- 弹窗逐项列出读到的值，**默认全选、可单独取消**；未勾选的字段保持当前设置不变。
  图片里没有的字段（多数外部图没有模型与 CFG）、以及本部署不支持的模型或采样器，
  会明确写在弹窗里而不是静默忽略。
- 拖动历史图走的是会话里存的原始参数，而不是重新解析像素 —— 后者会丢失编码时
  被丢弃的信息，开启「压缩体积」后导出的 JPEG 更是完全没有参数块。
- NAI 的 `scale` 与 WebUI 的 `CFG scale` 都是引导强度，对应本站的 Scale。
  NAI 的 `cfg_rescale` 是 0–1 的另一个量，**不会**被写进本站 0–30 的 CFG。
- 采样器名会做映射（`Euler a` → `k_euler_ancestral`、`DPM++ 2M Karras` →
  `k_dpmpp_2m` 等）；无法映射时该行不出现。

## 串行批量与生成锁

- 参数面板可设置任意正整数张数，不设固定上限。每张完成后才会提交下一张，不会并行请求。
- 生成按钮始终显示本次数量（`生成 ×N`）；运行时显示剩余数量并逐张递减。
- 留空 Seed 时每张使用独立随机 Seed；填写 Seed 时从该值开始逐张加 1（超过 uint32
  上限后回到 0）。
- 任意一张失败会停止剩余队列；「取消」会中止当前请求并清空尚未提交的队列。
- 页面会同时读取图片额度与临时 Key 全局日额度；图片已用大于 400 且全局剩余低于
  300 时显示警告。全局剩余低于 300 或额度暂不可用时，串行批量固定为单张；额度
  恢复后还原原设置。
- 批量运行期间会在内存中保留本批结果用于合成；关闭「加入历史」时，完成后只留下
  最新的对比图。
- 生成期间历史栏顶部有一个占位符，显示进度与目标分辨率；即使中途点开旧图回看，
  它也一直在，不会让人以为任务已经结束。**点它即可回到生成中的视图**（也可按
  <kbd>Esc</kbd>）。浏览旧图期间新图完成不会强行抢走画面，只更新历史栏。
- 占位符与舞台骨架屏的形状在请求发出时就锁定。生成中改画幅只影响下一次请求，
  不会让占位符变成这张图不可能有的尺寸。
- 两张及以上全部完成后，会在浏览器本地合成一张带序号与 Seed 的网格对比图，
  作为最后一个历史项展示和下载；合成过程不会再次请求上游或消耗额度。
- 同一页面有显式任务锁；同源的多个标签页再通过 Web Locks 互斥。本地代理还在
  Node 进程中强制只允许一个上游图片请求，冲突请求返回 HTTP 429。

截至 2026-08-22，Scylla 的模型接口没有公布并发额度、速率限制或任务锁字段，
响应也没有 `RateLimit` / `Retry-After` 一类头；公开的文档路径未提供可读取的并发契约。
这表示 HTTP 层可以发出并行请求，但不能据此保证同一 Key 的并行任务会执行、排队还是
被拒绝。本站因此采用保守的单并发策略；它只能约束同源标签页和同一个本地服务进程，
无法替 Scylla 锁住其他设备或其他客户端。

## Danbooru Tag 词典

- 描述与排除输入框支持中文、英文 Tag 自动补全；按热度最多显示 20 条，浮层内可滚动。
- 顶栏与描述栏的「查词」在**当前页面**打开查词面板：点一下结果即插入描述末尾，
  再点一下移除，无需切换标签页或二次确认。已在描述里的词（包括手打的）会标记为
  已选中。插入与移除都是就地拼接，不会破坏换行或 `1.3::tag::` 权重写法。
- 顶栏或描述栏的「Tag 词典」可打开独立搜索页，支持中英匹配、分类与使用量展示，
  每页 25 条且可持续翻页；可复制单个 Tag 或打开对应 Danbooru 作品页。
- 点击搜索结果的名称区即可加入 Prompt 组合侧栏，逐项设置权重后可「复制」，
  或「应用到生成器」——组合会暂存到本地，回到生成器标签页时询问是否追加到
  描述末尾；确认后才写入，超过 30 分钟的暂存会自动作废。
- 组合保存在浏览器本地，刷新不丢；清空前会确认。
- 类型筛选会在搜索页和输入补全之间同步；通用、作品、角色默认开启，画师与元数据
  默认关闭，可按需开启。
- 左栏「设置」里可关闭 Tag 自动补全。关闭后会真正卸载监听，输入时不再向词典
  服务发出任何请求，也不再弹出候选；该开关独立持久化，不受「恢复默认」影响。
- 描述和排除栏的「清洗格式」会保留换行，清理转义下划线、HTML 空格与续行符，
  并将 `(tag)`、`((tag))`、`(tag:0.9)` 转为 NAI 数值权重；角色、作品和画师的消歧括号会保留。
- 双语结果来自 `tagsuggest.zeabur.app`，英文查询必要时回退 Danbooru 官方自动补全接口。
  查询词会发送给对应在线服务，不会携带 Scylla API Key 或生成参数。

## NAI 工作区

`index.html` 从 `GET /v1/image/models` 读取并过滤 NovelAI 模型。其他绘图模型在独立的对话绘图页使用，不把 NAI 的采样器、步数、CFG 参数发送给它们。

## 两个实测结论

**上游用 HTTP 200 表示失败。** 参数非法时（例如分辨率超限）返回的是一张
512×256 的 PNG，像素内容是「Generation Failed」字样。前端已按尺寸特征拦截，
并在提交前按当前目录的 `max_resolution` 做分辨率守卫；较大画幅是否免费另由服务器报价决定。

**seed 不保证可复现。** 两次 `seed` 相同、`cache: false` 的请求返回了不同的
图像数据。因此 seed 由前端生成并记录，仅用于溯源，不承诺重现同一张图。

## 移动端

单列布局，顺序为 图片 → 历史 → 控制项：

- 历史在图片下方变为横向缩略图条，可横滑
- 生成按钮吸底，44px 触控高度，避让 iOS home indicator（`env(safe-area-inset-bottom)`）
- 用 `dvh` 而非 `vh`，避免地址栏收起时的高度跳动
- 触屏下不显示悬停提示，改用 `aria-label`；删除按钮与图片元信息常驻显示
- 拖拽分栏在触屏隐藏（无指针可拖）

## 快捷键

| 键 | 作用 |
| --- | --- |
| `⌘/Ctrl + Enter` | 生成 |
| `Esc` | 关闭放大视图 / 取消生成 |

## 验证

`npm run check` 检查全部前端模块语法并运行参数、额度、上下文和代理测试。
`npm run test:browser` 需要额外安装 Playwright 及 Chromium（或通过 `PLAYWRIGHT_MODULE` 指定模块路径）。
浏览器测试使用本地模拟上游，覆盖三轮改图、分支、多图、版本、取消、解码失败、报价变化、跨页锁、320/390/768px 和静态子路径直连，不消耗真实出图额度。

接口目录、周额度、生成/编辑报价已做线上只读核验。模拟测试不能证明真实模型出图质量；Gemini 编辑尺寸的实际落地结果及新旧路径临时点数计费仍需真实出图联调。

## 结构

```
docs/               ← GitHub Pages 根目录
  index.html
  chat.html          对话绘图页
  tags.html          中英 Danbooru Tag 搜索页
  ui.css             共享主题、字体、图标按钮与等待态
  styles.css         NAI 工作区布局
  chat.css           对话绘图布局与移动端适配
  tags.css           Tag 搜索页样式
  .nojekyll         ← 避免 Jekyll 处理下划线开头的路径
  js/
    app.js          主控：状态机、校验、历史、灯箱
    api.js          NAI 参数与新版媒体接口适配
    transport.js    本地代理 / 静态直连
    media-api.js    目录、额度、报价、执行与图片解码
    model-catalog.js  模型能力与参数校验（前后端共用）
    chat-page.js    对话工作区交互
    conversation-store.js  会话、上下文、分支与版本
    image-attachments.js   图片引用与内存资产管理
    generation-lock.js    跨工作区生成锁
    tag-api.js      双语 Tag 查询 + Danbooru 英文回退
    tag-autocomplete.js  Prompt 自动补全
    tags-page.js    Tag 搜索、分页与 Prompt 组合
    tag-picker.js   页内查词面板：点击即插入 / 移除描述
    tag-handoff.js  组合交接给生成器（本地暂存，30 分钟过期）
    image-meta.js   读取 PNG/JPEG 里的 NovelAI 与 WebUI 生成参数
    prompt-converter.js  WebUI 括号权重 → NAI 数值权重
    highlight.js    NAI 权重语法高亮
    toast.js        NAI / Tag 页共用的浮层提示
    vault.js        Key 加密保管库
    store.js        会话内历史 + 偏好
server.mjs          可选的本地服务（静态托管 + 代理）
```

## 权重语法高亮

输入框会实时给带权重的片段上色（黑底：偏红=加权，偏蓝=降权，越亮幅度越大）：

| 语法 | 权重 |
| --- | --- |
| `5::text::` | 5 |
| `-5::text::` | −5 |
| `{text}` | ×1.05，可嵌套 |
| `[text]` | ÷1.05，可嵌套 |

`()` 不视为权重语法——真实 tag 本身就含括号（如 `sho_(sho_lwlw)`）。
